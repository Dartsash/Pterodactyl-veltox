    ...
    return (
        <GreyRowBox css={tw`flex-wrap md:flex-nowrap items-center`} className={className}>
            <div css={tw`flex items-center truncate w-full md:flex-1`}>
                <div css={tw`mr-4`}>
                    {backup.completedAt !== null ? (
                        backup.isLocked ? (
                            <FontAwesomeIcon icon={faLock} css={tw`text-warning-400`} />
                        ) : (
                ...
                <div css={tw`flex flex-col truncate`}>
                    <div css={tw`flex items-center text-sm mb-1`}>
                        {backup.completedAt !== null && !backup.isSuccessful && (
                            <span
                                css={tw`bg-danger-600 py-px px-2 rounded-full text-white text-xs uppercase border border-danger-500 mr-2`}
                            >
                                Failed
                            </span>
                        )}
            ...
            <div css={tw`flex-1 md:flex-none md:w-48 mt-4 md:mt-0 md:ml-8 md:text-center`}>
                ...
                <p css={tw`text-2xs font-bold text-gray-500 uppercase tracking-wider mt-1`}>Created</p>
            </div>
