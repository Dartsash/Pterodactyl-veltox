...
const ScreenBlock = ({ title, image, message, onBack, onRetry }: ScreenBlockProps) => (
    <PageContentBlock>
        <div css={tw`flex justify-center`}>
            <div
                css={tw`w-full sm:w-3/4 md:w-1/2 p-12 md:p-20 bg-surface border border-gray-700 rounded-xl shadow-lg text-center relative`}
            >
                ...
                <h2 css={tw`mt-10 text-gray-50 font-bold text-4xl`}>{title}</h2>
                <p css={tw`text-sm text-gray-400 mt-2`}>{message}</p>
            </div>
        </div>
    </PageContentBlock>
);
