            ...
            <div class="box">
                ...
                <div class="box-body">
                    @if (auth()->user()->root_admin)
                        ...
                        <div class="form-group">
                            <label for="staff_role_id" class="control-label">Staff role</label>
                            <div>
                                <select name="staff_role_id" class="form-control">
                                    <option value="">None</option>
                                    @foreach (($staffRoles ?? collect()) as $role)
                                        <option value="{{ $role->id }}" {{ $user->staff_role_id === $role->id ? 'selected="selected"' : '' }}>{{ $role->name }}</option>
                                    @endforeach
                                </select>
                                <p class="text-muted"><small>
                                    Gives limited admin access based on the sections allowed by the role.
                                    Manage roles under <a href="{{ route('admin.addons.permissions') }}">Addons &rarr; Permission Management</a>.
                                    Ignored for full administrators.
                                </small></p>
                            </div>
                        </div>
                    @else
                        <p class="text-muted no-margin"><small>Only a root administrator can change administrator status or staff roles.</small></p>
                    @endif
                </div>
            </div>
        </div>
