...
class AuthenticateServerAccess
{
    ...
    public function handle(Request $request, \Closure $next): mixed
    {
        ...
        // Staff members with the "Servers" section granted by the Permission
        // Management addon get the same access as a root administrator here.
        $isServerStaff = $this->staff->canManageServers($user);

        if ($user->id !== $server->owner_id && !$user->root_admin && !$isServerStaff) {
        ...
        try {
            ...
            if (!$request->routeIs('api:client:server.view')) {
                ...
                if ((!$user->root_admin && !$isServerStaff) || !$request->routeIs($this->except)) {
                    throw $exception;
                }
            }
        }
