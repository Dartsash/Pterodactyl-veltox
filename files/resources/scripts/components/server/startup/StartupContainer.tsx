...
import StartupCommandBox from '@/components/server/startup/StartupCommandBox';
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Startup Settings'} showFlashKey={'startup:image'}>
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Startup Command'} css={tw`flex-1`}>
                    <div css={tw`px-1 py-2`}>
                        <p css={tw`font-mono bg-canvas border border-gray-700 rounded-lg py-2 px-4`}>{data.invocation}</p>
                    </div>
                </TitledGreyBox>
            ...
            {data.editor && (
                <StartupCommandBox
                    uuid={uuid}
                    rawStartupCommand={data.rawStartupCommand || ''}
                    editor={data.editor}
                    onUpdated={(invocation) => {
