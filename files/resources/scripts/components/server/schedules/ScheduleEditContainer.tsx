...
const CronBox = ({ title, value }: { title: string; value: string }) => (
    <div css={tw`bg-surface border border-gray-700 rounded-lg p-3`}>
...
const ActivePill = ({ active }: { active: boolean }) => (
    <span
        css={[
            ...
            active ? tw`bg-success-600 text-white` : tw`bg-danger-600 text-white`,
        ]}
    >
    ...
    return (
        <PageContentBlock title={'Schedules'}>
            ...
            {!schedule || isLoading ? (
                ...
                <>
                    <ScheduleCronRow cron={schedule.cron} css={tw`sm:hidden bg-surface border border-gray-700 rounded-lg mb-4 p-3`} />
                    <div css={tw`rounded shadow`}>
                        <div
                            css={tw`sm:flex items-center bg-raised p-3 sm:p-6 border-b border-gray-700 rounded-t-lg`}
                        >
                            <div css={tw`flex-1`}>
                                <h3 css={tw`flex items-center text-gray-100 text-2xl`}>
                                    ...
                                    {schedule.isProcessing ? (
                                        <span
                                            css={tw`flex items-center rounded-full px-2 py-px text-xs ml-4 uppercase bg-raised text-gray-300`}
                                        >
                                ...
                                <p css={tw`mt-1 text-sm text-gray-200`}>
                                    ...
                                    <span css={tw`ml-4 pl-4 border-l-4 border-gray-700 py-px`}>
                                        Next run at:&nbsp;
                                        {schedule.nextRunAt ? (
                        ...
                        <div css={tw`bg-surface rounded-b-lg`}>
